...
#### S1B
```{r S1B, warning = FALSE,message=FALSE,fig.height = 4,fig.width=5}
replicate.sd<-list()
cell.sd<-list()
...
for(i in 2:(length(new.names)+1)){
  ...
  tmp<-tmp[which(toupper(tmp$X2)!='H446'),]
...
diversity.plot<-function(sheet,cell_lines,tracer){
  ...
  if(tracer=='glucose'){
    title<-expression('citrate labeling by [U-'^{13}*'C]glucose')
  }else{
    title<-expression('citrate labeling by [U-'^{13}*'C]glutamine')
  }
